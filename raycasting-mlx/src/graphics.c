	...
	initializeWindow(t_cub3d *cub3d)
{
	...
	win = mlx_new_window(mlx, cub3d->config->requested_width, cub3d->config->requested_height, "cub3d");
	if (!win)
		ft_exit_error(cub3d, "ERROR: MLX_NEW_WINDOW ERROR.");
	cub3d->mlx.mlx = mlx;
	cub3d->win = win;
	cub3d->image.img_ptr = mlx_new_image(mlx, cub3d->config->requested_width, cub3d->config->requested_height); //check for leak
	...
	clearColorBuffer(t_cub3d *cub3d, uint32_t color)
{
	...
	i = 0;
	while (i < cub3d->config->requested_width * cub3d->config->requested_height)
	{
