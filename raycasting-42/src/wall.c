...
/*   Updated: 2021/06/15 14:59:07 by asydykna         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	perform_dda(t_cub3d *cub3d)
{
	while (cub3d->config->hit == 0)
	{
		...
		if (cub3d->config->map[(cub3d->screen.mapX)
				+ (cub3d->screen.mapY * cub3d->config->max_len)] == '1')
			cub3d->config->hit = 1;
	}
}
	...
	calc_ray(t_cub3d *cub3d, int x)
{
	...
	if (cub3d->wall.rayDirY == 0)
		cub3d->wall.deltaDistX = 0;
	else
	{
		if (cub3d->wall.rayDirX == 0)
			cub3d->wall.deltaDistX = 1;
		else
			cub3d->wall.deltaDistX = fabs(1 / cub3d->wall.rayDirX);
	}
	if (cub3d->wall.rayDirX == 0)
		cub3d->wall.deltaDistY = 0;
	else
	{
		if (cub3d->wall.rayDirY == 0)
			cub3d->wall.deltaDistY = 1;
		else
